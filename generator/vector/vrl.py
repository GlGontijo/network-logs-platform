...
def _vrl_date_time_to_iso(src: str, dst: str, fm: FieldMapping) -> list[str]:
    ...
    # do FortiOS em um timestamp ISO 8601 real, corrigindo o timezone do
    # dispositivo via aritmética de offset (não usamos o parâmetro
    # `timezone:` de parse_timestamp diretamente — o parser de string de tz
    # do VRL rejeita formatos como "-03:00"; segundos é mais confiável).
    #
    # Estratégia:
    #   1. Faz o parse de date+time como se fosse UTC (naive) -> _naive_epoch
    #   2. Lê '.tz' do FortiOS (formato "-0300"/"+0200") e calcula o offset
    #      em segundos
    #   3. Timestamp real = _naive_epoch - offset_segundos
    #      (local = UTC + offset  =>  UTC = local - offset)
    #   4. Sem '.tz' presente, assume UTC (offset = 0) — nunca usa o
    #      timezone do host rodando o Vector.
    return [
        f"if exists(.date) && exists(.time) {{",
        f'    _combined = (to_string(.date) ?? "") + " " + (to_string(.time) ?? "")',
        f'    _naive, err = parse_timestamp(_combined, format: "%Y-%m-%d %H:%M:%S", timezone: "UTC")',
        f"    if err == null {{",
        f"        _offset_seconds = 0",
        f"        if exists(.tz) {{",
        f'            _tz_raw = to_string(.tz) ?? ""',
        f"            if length(_tz_raw) == 5 {{",
        f'                _sign = slice!(_tz_raw, 0, 1)',
        f"                _hh, err_h = to_int(slice!(_tz_raw, 1, 3))",
        f"                _mm, err_m = to_int(slice!(_tz_raw, 3, 5))",
        f"                if err_h == null && err_m == null {{",
        f"                    _offset_seconds = (_hh * 3600) + (_mm * 60)",
        f'                    if _sign == "-" {{',
        f"                        _offset_seconds = -_offset_seconds",
        f"                    }}",
        f"                }}",
        f"            }}",
        f"        }}",
        f'        _naive_epoch = to_unix_timestamp(_naive, unit: "seconds")',
        f"        _real_epoch = _naive_epoch - _offset_seconds",
        f'        {dst} = from_unix_timestamp!(_real_epoch, unit: "seconds")',
        f"    }} else {{",
