...
class VendorValidator:
    ...
    def _validate_row(self, row: dict, row_idx: int, report: ValidationReport) -> None:
        ...
        required_str = _s("required").lower()
        ...
        if transform != "derived":
            # 'labels' é um field set ECS de objeto livre (flattened) — qualquer
            # sub-chave é válida por design e nunca aparece listada no fields.csv,
            # então pulamos a checagem de existência para esse prefixo.
            is_free_labels_field = ecs_field.startswith("labels.")

            # ── ecs_field existe no registry ─────────────────────────────────
            if not is_free_labels_field and not self._reg.exists(ecs_field):
            ...
            if is_free_labels_field:
                # labels.* aceita qualquer tipo escalar (string/number/bool);
                # não há tipo ECS fixo para validar contra.
                return
